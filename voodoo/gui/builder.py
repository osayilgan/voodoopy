...
class Page:
    def __enter__(self):
        ...
        self.state = State()  # State nesnesi ekliyoruz
        return self
    ...
    def input(self, id=None, text=None):
        ...
        # HTML input elementi oluşturuluyor ve state'e kaydediliyor
        self.content += f'<input id="{id}" class="border border-gray-300 p-2 rounded-md w-full" type="text" value="{text_value}">'
        self.state.set(id, text_value)  # Input değerlerini state'e kaydediyoruz
    ...
    def submit_data(self, function_ref, state):
        """Fonksiyon adı yerine direkt fonksiyon referansı alır."""
        try:
            if callable(function_ref):  # Eğer geçilen değer çağrılabilir bir fonksiyon ise çalıştır
                function_ref(state)
            else:
